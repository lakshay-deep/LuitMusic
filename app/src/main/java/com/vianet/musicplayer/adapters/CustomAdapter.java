...
/**
 * Created by editing2 on 09-Nov-16.
 */

public class CustomAdapter extends BaseAdapter {

    private Context context;
    private int selectData, arraySize;

    public CustomAdapter(Context context, int arraySize, int selectData) {
        this.context = context;
        this.selectData = selectData;
        this.arraySize = arraySize;
//        this.fraBase = fraMain;
    }
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        switch (selectData) {
            case 0:
                networkImageView.setImageUrl(LandingPage.albumThumb.get(position), loadImage);
                networkImageView.setScaleType(ImageView.ScaleType.FIT_XY);
                textViewEven.setText(LandingPage.album.get(position));
                break;
            case 1:
                networkImageView.setImageUrl(LandingPage.artistThumb.get(position), loadImage);
                networkImageView.setScaleType(ImageView.ScaleType.FIT_XY);
                textViewEven.setText(LandingPage.artist.get(position));
                break;
            case 2:
                networkImageView.setImageUrl(JsonParse1.thumbnail[position], loadImage);
                networkImageView.setScaleType(ImageView.ScaleType.FIT_XY);
                textViewEven.setText(JsonParse1.title[position]);
                break;
        }
        return view;
    }
